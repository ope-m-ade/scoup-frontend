...
export default function EditPatentForm() {
    ...
    const loadPatent = async () => {
      ...
      try {
        const res = await fetchWithAuth(
          `${API_BASE_URL}/faculty/patents/${id}/`
        );
        if (!res.ok) {
          throw new Error("Unable to load patent.");
        }
        ...
        setForm({
          ...
          issue_date: toDateInput(data.issue_date),
          link: data.link || "",
        });
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Unexpected error loading patent."
        );
      } finally {
        setLoading(false);
      }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data?.detail ||
            data?.error ||
            (isEdit ? "Unable to save patent." : "Unable to add patent.")
        );
      }
      navigate("/faculty/patents");
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unexpected error saving patent."
      );
    } finally {
      setSaving(false);
    }
  ...
  if (loading) {
    return <p className="text-sm text-gray-600">Loading patent...</p>;
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle>{isEdit ? "Edit Patent" : "Add Patent"}</CardTitle>
